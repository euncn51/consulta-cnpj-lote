...
class CNPJConsultorGUI:
    ...
    def _create_help_tab(self):
        ...
        help_text = (
            "Bem-vindo ao Consultor de CNPJ v.3!\n\n"
            "• Consulta Individual:\n"
    ...
    def _display_result(self, resultado):
        ...
        # CORREÇÃO: Mostra detalhes das IEs apenas se existirem e não forem array vazio
        if 'INSCRICOES_ESTADUAIS' in resultado and resultado['INSCRICOES_ESTADUAIS'] and resultado['INSCRICOES_ESTADUAIS'] != '[]':
            try:
                ies = json.loads(resultado['INSCRICOES_ESTADUAIS'])
                if ies:  # Só mostra se houver inscrições
                    ...
                    self.result_text.insert(END, f"\n{LANG['ies_title']}\n")
                    self.result_text.insert(END, f"{LANG['ies_none']}\n")
            except json.JSONDecodeError:
                self.result_text.insert(END, f"\n{LANG['ies_error']}\n")
            except Exception as e:
                self.result_text.insert(END, f"\n{LANG['ies_error']}: {str(e)}\n")
        else:
            # Se não há inscrições estaduais, mostra mensagem informativa
            self.result_text.insert(END, f"\n{LANG['ies_title']}\n")
            self.result_text.insert(END, f"{LANG['ies_none']}\n")
