...
def parse_cnpj_list(text):
    """Extrai CNPJs de um texto (separados por linha, vírgula ou ponto-e-vírgula)"""
    # Remove espaços extras e quebras de linha
    text = text.strip()
    
    # Separa por linha, vírgula ou ponto-e-vírgula
    cnpjs = re.split(r'[\n,;]+', text)
    
    # Limpa e filtra CNPJs vazios
...
def tab_batch():
    ...
    if input_method == "📁 Upload de Arquivo":
        ...
        st.info("💡 Informe sua lista de CNPJs, você pode copiá-los de um arquivo de texto ou planilha")
        
        # Área de texto para colar CNPJs
        cnpj_text = st.text_area(
            "Cole os CNPJs aqui (um por linha ou separados por vírgula/ponto-e-vírgula):",
            height=200,
            placeholder="Exemplo:\n12.345.678/0001-90\n98.765.432/0001-10\nou\n12345678000190, 98765432000110",
            help="Você pode colar CNPJs com ou sem formatação, separados por linha, vírgula ou ponto-e-vírgula"
        )
        ...
        if st.button("🚀 Processar CNPJs", type="primary", use_container_width=True):
            if not cnpj_text.strip():
                st.error("❌ Cole pelo menos um CNPJ!")
            else:
                # Extrair CNPJs do texto
                cnpj_list = parse_cnpj_list(cnpj_text)
                
                if not cnpj_list:
                    st.error("❌ Nenhum CNPJ válido encontrado no texto!")
                else:
                    ...
                    for index, cnpj in enumerate(cnpj_list, 1):
                        if st.session_state.consultor.stop_processing:
                            st.warning("⚠️ Processamento cancelado!")
                            break
                        
                        cnpj_limpo = st.session_state.consultor.clean_cnpj(cnpj)
                    ...
                    st.success(f"✅ Processamento concluído! {len(resultados)} CNPJs processados.")
                    add_log(f"Processamento em lote (colado) concluído: {len(resultados)} CNPJs", "success")
...
def tab_help():
    ...
    with st.expander("🔍 Consulta Individual", expanded=True):
        ...
        5. **Exporte para Excel** clicando no botão de exportação
    ...
    with st.expander("📊 Processamento em Lote"):
        ...
        **Método 2: Colar CNPJs** ⭐ NOVO!
        
        1. **Copie os CNPJs** de qualquer fonte (Excel, texto, etc.)
        
        2. **Cole no campo de texto** - aceita:
           - Um CNPJ por linha
           - CNPJs separados por vírgula
           - CNPJs separados por ponto-e-vírgula
           - Com ou sem formatação
        
        3. **Defina o nome do arquivo de saída**
        
        4. **Clique em Processar CNPJs** e aguarde
        
        5. **Baixe os resultados** usando o botão de download
        ...
        **Observações:**
        ...
        - Preview mostra os primeiros 20 resultados
        - Estatísticas mostram total, erros e sucessos
        """)
    ...
    with st.expander("🚀 Dicas e Atalhos"):
        ...
        - **Preview otimizado:** CNPJ formatado corretamente (sem notação científica)
        - **Colar CNPJs:** ⭐ Novo! Cole diretamente sem precisar criar arquivo
    ...
    **Versão:** 3.1 (Streamlit v2 - com opção de colar CNPJs)  
...
def main():
    ...
    # Título principal
    st.markdown('<h1 class="main-header">🏢 Consulta CNPJ</h1>', unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        "Consultor de CNPJ v3.1 | Desenvolvido com Streamlit | ⭐ Nova funcionalidade: Colar CNPJs"
        "</div>",
        unsafe_allow_html=True
    )
