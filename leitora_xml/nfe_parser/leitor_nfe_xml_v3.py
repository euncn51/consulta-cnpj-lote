    ...
    def parse_gtribregular(self, icms_elem, icms_type, item_data):
        ...
        try:
            ...
            if gTribRegular is not None:
                gtrib_fields = {
                    f'ICMS_{icms_type}_gTribRegular_CSTReg': 'CSTReg',
                    f'ICMS_{icms_type}_gTribRegular_cClassTribReg': 'cClassTribReg',
                    f'ICMS_{icms_type}_gTribRegular_pAliqEfetRegIBSUF': 'pAliqEfetRegIBSUF',
                    f'ICMS_{icms_type}_gTribRegular_vTribRegIBSUF': 'vTribRegIBSUF',
                    f'ICMS_{icms_type}_gTribRegular_pAliqEfetRegIBSMun': 'pAliqEfetRegIBSMun',
                    f'ICMS_{icms_type}_gTribRegular_vTribRegIBSMun': 'vTribRegIBSMun',
                    f'ICMS_{icms_type}_gTribRegular_pAliqEfetRegCBS': 'pAliqEfetRegCBS',
                    f'ICMS_{icms_type}_gTribRegular_vTribRegCBS': 'vTribRegCBS'
                }
...
# =============================================================================
# FUNÇÕES DE EXPORTAÇÃO ORGANIZADAS
# =============================================================================
...
def ordenar_colunas_por_schema(df):
    ...
    ordem_schema = [
        ...
        'Id', 'versao_nfe', 'cUF', 'cNF', 'natOp', 'mod', 'serie', 'nNF', 'dhEmi', 'dhSaiEnt',
        'tpNF', 'idDest', 'cMunFG', 'cMunFGIBS', 'tpImp', 'tpEmis', 'cDV', 'tpAmb', 'finNFe',
        'indFinal', 'indPres', 'procEmi', 'verProc', 'tpNFDebito', 'tpNFCredito',
        
        # 2. EMITENTE
        'emit_CNPJ', 'emit_CPF', 'emit_xNome', 'emit_xFant', 'emit_IE', 'emit_IEST', 'emit_IM',
        'emit_CNAE', 'emit_CRT', 'emit_ender_xLgr', 'emit_ender_nro', 'emit_ender_xBairro',
        'emit_ender_cMun', 'emit_ender_xMun', 'emit_ender_UF', 'emit_ender_CEP',
        
        # 3. DESTINATÁRIO
        'dest_CNPJ', 'dest_CPF', 'dest_idEstrangeiro', 'dest_xNome', 'dest_indIEDest', 'dest_IE',
        'dest_ISUF', 'dest_IM', 'dest_email', 'dest_ender_xLgr', 'dest_ender_nro', 'dest_ender_xBairro',
        'dest_ender_cMun', 'dest_ender_xMun', 'dest_ender_UF', 'dest_ender_CEP',
        
        # 4. ITENS
        'nItem', 'cProd', 'xProd', 'NCM', 'CFOP', 'uCom', 'qCom', 'vUnCom', 'vProd', 'uTrib', 
        'qTrib', 'vUnTrib', 'vFrete', 'vSeg', 'vDesc', 'vOutro', 'indTot', 'tpCredPresIBSZFM',
        'gCred_cCredPresumido', 'gCred_pCredPresumido', 'gCred_vCredPresumido', 'vItem', 'vTotTrib',
        ...
        'total_vOutro', 'total_vNF', 'total_vTotTrib', 'total_vTotTribIBSUF', 'total_vTotTribIBSMun',
        'total_vTotTribCBS',
        
        # 6. TRANSPORTE
        'transp_modFrete', 'transp_transporta_xNome', 'transp_transporta_IE',
        
        # 7. PAGAMENTO
        'pag_detPag_tPag', 'pag_detPag_vPag'
    ]
...
def criar_aba_estatisticas(df):
    ...
    if 'ibscbs_CST' in df.columns:
        ...
        estatisticas.append({'Estatística': 'Total de Itens analisados', 'Valor': len(df)})
...
def criar_aba_detalhes_icms(df):
    """Cria aba com detalhes do ICMS"""
    icms_colunas = [col for col in df.columns if 'ICMS_' in col]
    if icms_colunas:
        colunas_base = ['nNF', 'nItem', 'xProd', 'vProd']
...
def criar_aba_detalhes_ibscbs(df):
    """Cria aba com detalhes do IBSCBS"""
    ibscbs_colunas = [col for col in df.columns if 'ibscbs_' in col]
    if ibscbs_colunas:
        colunas_base = ['nNF', 'nItem', 'xProd', 'vProd']
        colunas_detalhes = colunas_base + ibscbs_colunas
        return df[colunas_detalhes]
    else:
        return pd.DataFrame({'Mensagem': ['Nenhum dado de IBSCBS encontrado']})
...
if __name__ == "__main__":
    ...
    print("INCLUINDO VALIDAÇÃO COMPLETA DO IBSCBS")
    ...
    if not df.empty:
        ...
        print(f"\n ARQUIVOS EXPORTADOS:")
        print(f"   Excel: {excel_file} (com 5 abas organizadas)")
        ...
        print("   ABA 5: Detalhes_IBSCBS - Foco em informações do IBSCBS")
        
        # Mostrar amostra
        print(f"\n AMOSTRA DOS DADOS (primeiras 3 linhas):")
        colunas_amostra = ['nItem', 'xProd', 'vProd', 'vItem', 'total_vNF', 'idDest']
