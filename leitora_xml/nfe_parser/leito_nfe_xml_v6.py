...
def criar_aba_estatisticas(df):
    """Cria aba de estatísticas para o Excel - INCLUINDO TOTAIS IBSCBS"""
    estatisticas = []
    ...
    # =============================================================================
    # TOTAIS IBSCBS - ADICIONADOS
    # =============================================================================
    
    # Totais IBSCBS do grupo total (IBSCBSTot)
    campos_ibscbs_tot = {
        'total_vTotTribIBSUF': 'Total Tributo IBS UF (R$)',
        'total_vTotTribIBSMun': 'Total Tributo IBS Município (R$)',
        'total_vTotTribCBS': 'Total Tributo CBS (R$)'
    }
    
    for campo, nome in campos_ibscbs_tot.items():
        if campo in df.columns and df[campo].notna().any():
            estatisticas.append({'Estatística': nome, 'Valor': f"R$ {df[campo].sum():,.2f}"})
    
    # Totais IBSCBS por item (somando valores dos itens)
    if 'ibscbs_gIBSUF_vIBSUF' in df.columns and df['ibscbs_gIBSUF_vIBSUF'].notna().any():
        total_ibscbs_uf = df['ibscbs_gIBSUF_vIBSUF'].sum()
        estatisticas.append({'Estatística': 'Total IBS UF por Itens (R$)', 'Valor': f"R$ {total_ibscbs_uf:,.2f}"})
    
    if 'ibscbs_gIBSMun_vIBSMun' in df.columns and df['ibscbs_gIBSMun_vIBSMun'].notna().any():
        total_ibscbs_mun = df['ibscbs_gIBSMun_vIBSMun'].sum()
        estatisticas.append({'Estatística': 'Total IBS Município por Itens (R$)', 'Valor': f"R$ {total_ibscbs_mun:,.2f}"})
    
    if 'ibscbs_gCBS_vCBS' in df.columns and df['ibscbs_gCBS_vCBS'].notna().any():
        total_cbs = df['ibscbs_gCBS_vCBS'].sum()
        estatisticas.append({'Estatística': 'Total CBS por Itens (R$)', 'Valor': f"R$ {total_cbs:,.2f}"})
    
    # =============================================================================
    # TOTAIS GTRIBREGULAR - ADICIONADOS
    # =============================================================================
    
    if 'gTribRegular_vTribRegIBSUF' in df.columns and df['gTribRegular_vTribRegIBSUF'].notna().any():
        total_gtrib_uf = df['gTribRegular_vTribRegIBSUF'].sum()
        estatisticas.append({'Estatística': 'Total GTribRegular IBS UF (R$)', 'Valor': f"R$ {total_gtrib_uf:,.2f}"})
    
    if 'gTribRegular_vTribRegIBSMun' in df.columns and df['gTribRegular_vTribRegIBSMun'].notna().any():
        total_gtrib_mun = df['gTribRegular_vTribRegIBSMun'].sum()
        estatisticas.append({'Estatística': 'Total GTribRegular IBS Município (R$)', 'Valor': f"R$ {total_gtrib_mun:,.2f}"})
    
    if 'gTribRegular_vTribRegCBS' in df.columns and df['gTribRegular_vTribRegCBS'].notna().any():
        total_gtrib_cbs = df['gTribRegular_vTribRegCBS'].sum()
        estatisticas.append({'Estatística': 'Total GTribRegular CBS (R$)', 'Valor': f"R$ {total_gtrib_cbs:,.2f}"})
    ...
    if gtrib_campos:
        ...
        if campos_principais:
            ...
            estatisticas.append({'Estatística': 'Percentual com GTribRegular', 'Valor': f"{(count_gtrib/len(df))*100:.1f}%"})
...
def analisar_dados_completos(df):
    """Analisa todos os dados extraidos - INCLUINDO TOTAIS IBSCBS"""
    ...
    # =============================================================================
    # ANÁLISE IBSCBS - ATUALIZADA COM TOTAIS
    # =============================================================================
    print(f"\n ANÁLISE IBSCBS:")
    if 'ibscbs_CST' in df.columns:
        total_ibscbs = df['ibscbs_CST'].notna().sum()
        ...
        # Totais IBSCBS do grupo total
        campos_ibscbs_tot = {
            'total_vTotTribIBSUF': 'Total Tributo IBS UF',
            'total_vTotTribIBSMun': 'Total Tributo IBS Município', 
            'total_vTotTribCBS': 'Total Tributo CBS'
        }
        
        for campo, nome in campos_ibscbs_tot.items():
            if campo in df.columns and df[campo].notna().any():
                valor_total = df[campo].sum()
                print(f"   • {nome}: R$ {valor_total:,.2f}")
        
        # Totais IBSCBS por item
        if 'ibscbs_gIBSUF_vIBSUF' in df.columns and df['ibscbs_gIBSUF_vIBSUF'].notna().any():
            total_ibscbs_uf = df['ibscbs_gIBSUF_vIBSUF'].sum()
            print(f"   • Total IBS UF por Itens: R$ {total_ibscbs_uf:,.2f}")
        
        if 'ibscbs_gIBSMun_vIBSMun' in df.columns and df['ibscbs_gIBSMun_vIBSMun'].notna().any():
            total_ibscbs_mun = df['ibscbs_gIBSMun_vIBSMun'].sum()
            print(f"   • Total IBS Município por Itens: R$ {total_ibscbs_mun:,.2f}")
        
        if 'ibscbs_gCBS_vCBS' in df.columns and df['ibscbs_gCBS_vCBS'].notna().any():
            total_cbs = df['ibscbs_gCBS_vCBS'].sum()
            print(f"   • Total CBS por Itens: R$ {total_cbs:,.2f}")
    
    # =============================================================================
    # GTRIBREGULAR - ATUALIZADA COM TOTAIS
    # =============================================================================
    print(f"\n GTRIBREGULAR:")
    gtrib_campos = [col for col in df.columns if 'gTribRegular' in col]
    if gtrib_campos:
        ...
        if campos_principais:
            ...
            if 'gTribRegular_vTribRegIBSUF' in df.columns and df['gTribRegular_vTribRegIBSUF'].notna().any():
                total_gtrib_uf = df['gTribRegular_vTribRegIBSUF'].sum()
                print(f"   • Total GTribRegular IBS UF: R$ {total_gtrib_uf:,.2f}")
            
            if 'gTribRegular_vTribRegIBSMun' in df.columns and df['gTribRegular_vTribRegIBSMun'].notna().any():
                total_gtrib_mun = df['gTribRegular_vTribRegIBSMun'].sum()
                print(f"   • Total GTribRegular IBS Município: R$ {total_gtrib_mun:,.2f}")
            
            if 'gTribRegular_vTribRegCBS' in df.columns and df['gTribRegular_vTribRegCBS'].notna().any():
                total_gtrib_cbs = df['gTribRegular_vTribRegCBS'].sum()
                print(f"   • Total GTribRegular CBS: R$ {total_gtrib_cbs:,.2f}")
