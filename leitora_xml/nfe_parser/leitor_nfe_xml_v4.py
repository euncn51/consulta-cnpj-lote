...
def criar_aba_detalhes_icms(df):
    ...
    if icms_colunas:
        colunas_base = ['serie', 'nNF', 'nItem', 'cProd', 'xProd', 'NCM', 'CFOP', 'vProd']
...
def criar_aba_detalhes_ibscbs(df):
    ...
    if todas_colunas:
        colunas_base = ['serie', 'nNF', 'emit_CNPJ', 'cProd', 'xProd', 'NCM', 'CFOP']
...
def criar_aba_gtribregular(df):
    ...
    if gtrib_colunas:
        ...
        cstreg_cols = [col for col in gtrib_colunas if 'CSTReg' in col]
        if cstreg_cols:
            cstreg_col = cstreg_cols[0]
            df_gtrib['CSTReg_Desc'] = df_gtrib[cstreg_col].apply(
                lambda x: 'Alíquota reduzida' if x == '200' else 'Suspensão' if x == '550' else x
            )
