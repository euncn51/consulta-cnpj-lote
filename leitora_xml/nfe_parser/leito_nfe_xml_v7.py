...
class IBSCBSTot:
    ...
    vBCIBSCBS: Decimal
    gIBS: Optional[Any] = None
    gCBS: Optional[Any] = None
    gMono: Optional[Any] = None
    gEstornoCred: Optional[Any] = None
        ...
        try:
            ...
            for det in det_elements:
                ...
                if vItem:
                    ...
                    # Se vItem não estiver disponível, usar vProd como fallback
    ...
    def parse_totais(self, inf_nfe, header_data):
        ...
        try:
            ...
            if total is not None:
                ...
                self.parse_vnftot(total, header_data)
    ...
    def parse_ibscbs_tot(self, total, header_data):
        ...
        try:
            ...
            if IBSCBSTot is not None:
                ...
                # Parse dos subgrupos do IBSCBSTot
                self.parse_gibs(IBSCBSTot, header_data)
                self.parse_gcbs(IBSCBSTot, header_data)
                self.parse_gmono(IBSCBSTot, header_data)
                self.parse_gestornocred(IBSCBSTot, header_data)
                        
        except Exception as e:
            print(f"Erro ao parsear IBSCBSTot: {e}")

    def parse_gibs(self, ibscbstot, header_data):
        ...
        try:
            ...
            print(f"Erro ao parsear gIBS: {e}")

    def parse_gcbs(self, ibscbstot, header_data):
        ...
        try:
            ...
            print(f"Erro ao parsear gCBS: {e}")

    def parse_gmono(self, ibscbstot, header_data):
        ...
        try:
            ...
            print(f"Erro ao parsear gMono: {e}")

    def parse_gestornocred(self, ibscbstot, header_data):
        ...
        try:
            ...
            print(f"Erro ao parsear gEstornoCred: {e}")

    def parse_vnftot(self, total, header_data):
        ...
        try:
            ...
            print(f"Erro ao parsear vNFTot: {e}")
...
def ordenar_colunas_por_schema(df):
    ...
    ordem_schema = [
        ...
        # 4. ITENS - INCLUINDO vItem e vProd
        ...
        'total_vOutro', 'total_vNF', 'total_vTotTrib', 'vNFTot',
        
        # 5.2 IBSCBSTot - NOVOS CAMPOS
        'total_vBCIBSCBS', 'total_gIBS_vIBS', 'total_gIBS_vCredPres', 'total_gIBS_vCredPresCondSus',
        'total_gIBS_gIBSUF_vDif', 'total_gIBS_gIBSUF_vDevTrib', 'total_gIBS_gIBSUF_vIBSUF',
        'total_gIBS_gIBSMun_vDif', 'total_gIBS_gIBSMun_vDevTrib', 'total_gIBS_gIBSMun_vIBSMun',
        'total_gCBS_vDif', 'total_gCBS_vDevTrib', 'total_gCBS_vCBS', 'total_gCBS_vCredPres', 'total_gCBS_vCredPresCondSus',
        'total_gMono_vIBSMono', 'total_gMono_vCBSMono', 'total_gMono_vIBSMonoReten', 'total_gMono_vCBSMonoReten',
        'total_gMono_vIBSMonoRet', 'total_gMono_vCBSMonoRet',
        'total_gEstornoCred_vIBSEstCred', 'total_gEstornoCred_vCBSEstCred'
    ]
...
def criar_aba_estatisticas(df):
    ...
    # Valores totais - INCLUINDO vItem, vProd e vNFTot
    ...
    if 'vNFTot' in df.columns:
        estatisticas.append({'Estatística': 'Valor Total NFTot (R$)', 'Valor': f"R$ {df['vNFTot'].sum():,.2f}"})
...
def criar_aba_resumo(df):
    """Cria aba de resumo para o Excel - INCLUINDO vItem, vProd e vNFTot"""
    resumo_data = []
    ...
    if 'nNF' in df.columns:
        ...
        for _, nfe in nfes_agrupadas.iterrows():
            ...
            resumo_data.append({
                ...
                'Valor Produtos (vProd)': nfe['vProd'],
                'Valor Itens (vItem)': nfe['vItem'],
                'Valor NF (total_vNF)': nfe['total_vNF'],
                'Valor NFTot (vNFTot)': nfe['vNFTot']
            })
    
    return pd.DataFrame(resumo_data)

def criar_aba_detalhes_icms(df):
    """Cria aba com detalhes do ICMS - INCLUINDO vItem e vProd"""
    icms_colunas = [col for col in df.columns if 'ICMS_' in col]
    if icms_colunas:
        colunas_base = ['serie', 'nNF', 'emit_CNPJ', 'nItem', 'cProd', 'xProd', 'NCM', 'CFOP', 'vProd', 'vItem']
...
def criar_aba_detalhes_ibscbs(df):
    """Cria aba com detalhes do IBSCBS incluindo GTribRegular - INCLUINDO vItem, vProd E vNFTot"""
    ...
    if todas_colunas:
        colunas_base = ['serie', 'nNF', 'emit_CNPJ', 'cProd', 'xProd', 'NCM', 'CFOP', 'vProd', 'vItem', 'vNFTot']
...
def criar_aba_gtribregular(df):
    """Cria aba específica para GTribRegular com todas as informações - INCLUINDO vItem, vProd e vNFTot"""
    gtrib_colunas = [col for col in df.columns if 'gTribRegular' in col]
...
def analisar_dados_completos(df):
    ...
    # Valores monetários - INCLUINDO vItem, vProd e vNFTot
    print(f"\n VALORES MONETARIOS:")
    campos_valores = ['vProd', 'vItem', 'total_vNF', 'vNFTot']
...
if __name__ == "__main__":
    ...
    print("INCLUINDO CAMPOS vItem, vProd E vNFTot NAS ABAS SOLICITADAS")
    ...
    if not df.empty:
        ...
        print("   ABA 1: NFes_Completas - Todos os dados ordenados por schema (INCLUI vItem, vProd, vNFTot)")
        print("   ABA 2: Resumo_NFe - Resumo por nota fiscal (INCLUI vItem, vProd, vNFTot)") 
        print("   ABA 3: Estatisticas - Estatísticas consolidadas (INCLUI vItem, vProd, vNFTot)")
        print("   ABA 4: Detalhes_ICMS - Foco em informações do ICMS (INCLUI vItem, vProd)")
        print("   ABA 5: Detalhes_IBSCBS - IBSCBS + GTribRegular (INCLUI vItem, vProd, vNFTot)")
        print("   ABA 6: GTribRegular - Foco em tributação regular (INCLUI vItem, vProd, vNFTot)")
        
        # Mostrar amostra
        print(f"\n AMOSTRA DOS DADOS (primeiras 3 linhas):")
        colunas_amostra = ['nItem', 'xProd', 'vProd', 'vItem', 'total_vNF', 'vNFTot', 'idDest', 'emit_CNPJ', 'serie']
